...
def race_info(url:str)->list[str]:
    ...
    race_num = soup.find_all(class_ = "Race_Num Race_Fixed")[0].text.replace("\n","")
...
def format_result_race(url:str)->str:
    html = requests.get(url)
    soup = bs4.BeautifulSoup(html.content, 'html.parser')
    result = soup.find_all(class_ = "FullWrap clearfix")
    #レース情報の取得
    info = race_info(url)
    #単勝の結果取得
...
if __name__ == "__main__":
    format_result_race("https://nar.netkeiba.com/race/result.html?race_id=202250081010")
